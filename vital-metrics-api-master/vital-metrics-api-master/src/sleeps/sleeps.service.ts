...
import { sleeps, dailyMetrics } from '../drizzle/schema';
...
export class SleepsService {
  ...
  async create(userId: number, dto: CreateSleepDto) {
    ...
    await this.dailyMetricsService.syncSleepFromLogs(
      metric.metrics_id,
      userId,
    );
  ...
  async findAll(userId: number, page: number = 1, limit: number = 20) {
    ...
    const [data, [{ value: total }]] = await Promise.all([
      this.db.query.sleeps.findMany({
        where: inArray(sleeps.metrics_id, metricIds),
        limit,
        offset,
      }),
      this.db
  ...
  async findOne(userId: number, id: number) {
    const record = await this.db.query.sleeps.findFirst({
      where: eq(sleeps.sleep_id, id),
    });
  ...
  async update(userId: number, id: number, dto: UpdateSleepDto) {
    ...
    const [updated] = await this.db
      ...
      .set({
        ...
        quality: dto.quality ?? current.quality,
    ...
    if (metricsId !== current.metrics_id) {
      await this.dailyMetricsService.syncSleepFromLogs(metricsId, userId);
    }
