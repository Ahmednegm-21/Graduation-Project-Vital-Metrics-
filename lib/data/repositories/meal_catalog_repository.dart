...
class CatalogMeal {
  ...
  String get category {
    ...
    if (n.contains('oat') || n.contains('egg') || n.contains('toast') ||
        ...
        n.contains('breakfast') || n.contains('cereal'))        return 'breakfast';
    if (n.contains('steak') || n.contains('chicken') || n.contains('salmon') ||
        n.contains('pasta') || n.contains('pizza') || n.contains('burger') ||
        n.contains('ravioli') || n.contains('dinner'))          return 'dinner';
    if (n.contains('bar') || n.contains('nut') || n.contains('fruit') ||
        n.contains('chocolate') || n.contains('cookie') ||
        n.contains('snack') || n.contains('moo'))               return 'snacks';
    if (n.contains('salad') || n.contains('wrap') || n.contains('sandwich') ||
        n.contains('soup') || n.contains('bowl') || n.contains('lunch') ||
        n.contains('lentil') || n.contains('chil'))             return 'lunch';
    return 'lunch';
  }

  // ── Local fallback data — used when backend catalog is empty ──────────────
  static const List<CatalogMeal> localFallback = [
    CatalogMeal(mealId: -1,  name: 'Organic Old Fashioned Oats',  description: '', calories: 160, protein: 5,  carbs: 28, fat: 3,  isLocal: true),
    CatalogMeal(mealId: -2,  name: 'Spicy Lentil Wrap w/ Sauce',  description: '', calories: 580, protein: 22, carbs: 75, fat: 18, isLocal: true),
    CatalogMeal(mealId: -3,  name: 'TJ 4 Cheese Mini Ravioli',    description: '', calories: 250, protein: 10, carbs: 35, fat: 8,  isLocal: true),
    CatalogMeal(mealId: -4,  name: 'Chiles Rellenos Con Queso',   description: '', calories: 350, protein: 14, carbs: 20, fat: 22, isLocal: true),
    CatalogMeal(mealId: -5,  name: 'Organic Midnight Moo',        description: '', calories: 110, protein: 3,  carbs: 18, fat: 3,  isLocal: true),
    CatalogMeal(mealId: -6,  name: 'Chicken Tikka Masala',        description: '', calories: 350, protein: 28, carbs: 22, fat: 14, isLocal: true),
    CatalogMeal(mealId: -7,  name: 'Greek Yogurt with Honey',     description: '', calories: 180, protein: 12, carbs: 24, fat: 3,  isLocal: true),
    CatalogMeal(mealId: -8,  name: 'Avocado Toast',               description: '', calories: 290, protein: 8,  carbs: 30, fat: 16, isLocal: true),
    CatalogMeal(mealId: -9,  name: 'Caesar Salad',                description: '', calories: 220, protein: 9,  carbs: 14, fat: 15, isLocal: true),
    CatalogMeal(mealId: -10, name: 'Grilled Salmon',              description: '', calories: 410, protein: 46, carbs: 0,  fat: 24, isLocal: true),
    CatalogMeal(mealId: -11, name: 'Mixed Nuts',                  description: '', calories: 180, protein: 5,  carbs: 8,  fat: 15, isLocal: true),
    CatalogMeal(mealId: -12, name: 'Banana Smoothie',             description: '', calories: 240, protein: 6,  carbs: 45, fat: 4,  isLocal: true),
    CatalogMeal(mealId: -13, name: 'Scrambled Eggs',              description: '', calories: 200, protein: 14, carbs: 2,  fat: 15, isLocal: true),
    CatalogMeal(mealId: -14, name: 'Brown Rice Bowl',             description: '', calories: 320, protein: 6,  carbs: 68, fat: 2,  isLocal: true),
    CatalogMeal(mealId: -15, name: 'Protein Bar',                 description: '', calories: 210, protein: 20, carbs: 22, fat: 7,  isLocal: true),
  ];
}
