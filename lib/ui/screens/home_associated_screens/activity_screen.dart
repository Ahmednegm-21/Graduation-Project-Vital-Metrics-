...
class _TodayScreenView extends StatelessWidget {
  ...
  Future<void> _showDatePicker(BuildContext context) async {
    await showDatePicker(
      ...
      builder: (ctx, child) => Theme(
        data: Theme.of(ctx).copyWith(colorScheme: const ColorScheme.light(primary: Color(0xFF4361EE))),
        child: child!,
      ),
    );
  }
              ...
              child: BlocBuilder<ActivityCubit, ActivityState>(
                builder: (context, state) {
                  if (state is TodayLoading) {
                    return const Center(child: CircularProgressIndicator(color: Color(0xFF4361EE), strokeWidth: 2.5));
                  }
                  if (state is TodayError) return _buildError(context, state.message);
                  if (state is TodayLoaded) return _buildContent(context, state, isDark);
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        const Icon(Icons.calendar_month_rounded, color: Color(0xFF4361EE), size: 18),
                        SizedBox(width: 7.w),
                        Text(_todayLabel(),
                            style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13.sp, color: context.colors.text)),
                      ],
                    ),
                  ),
                ),
                SizedBox(height: 8.h),
                // ← StepsConnectToggle بدل HealthConnectToggle
                const StepsConnectToggle(),
              ],
            ),
          ),
          Row(
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(color: const Color(0xFFFF3B30).withOpacity(0.1), shape: BoxShape.circle),
              child: Icon(Icons.error_outline_rounded, size: 48.sp, color: const Color(0xFFFF3B30)),
            ),
            SizedBox(height: 16.h),
            Text(message, style: TextStyle(fontSize: 14.sp, color: const Color(0xFFFF3B30)), textAlign: TextAlign.center),
          ...
          children: [
            FadeInUp(
              ...
              child: ActivityLevelCard(
                ...
                onTap: () => _showActivityLevelSheet(context, stats.activityLevel, activityCubit),
              ),
            ),
            SizedBox(height: 8.h),
            FadeInUp(
              ...
              child: Column(
                children: [
                  Center(child: _RingsSection(stats: stats, isDark: isDark)),
            ...
            if (!stats.hasActivity)
              FadeInUp(delay: const Duration(milliseconds: 160), child: const EmptyStateWidget()),
            if (stats.trackedActivities.isNotEmpty) ...[
                  ...
                  child: Row(
                    children: [
                      Container(width: 3.w, height: 18.h,
                          decoration: BoxDecoration(color: const Color(0xFF4361EE), borderRadius: BorderRadius.circular(4.r))),
                      SizedBox(width: 10.w),
                      Text('Tracked Activities',
                          style: TextStyle(fontSize: 17.sp, fontWeight: FontWeight.w800,
                              color: context.colors.text, letterSpacing: -0.3)),
                      const Spacer(),
                      Text('${stats.trackedActivities.length} logged',
                          style: TextStyle(fontSize: 11.sp, color: context.colors.subText, fontWeight: FontWeight.w500)),
                    ],
                  ),
                ),
              ),
              ...
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return TrackedActivityCard(activity: activity, onDelete: () => activityCubit.removeActivity(activity.id));
                },
              ),
            ],
  ...
  void _showActivityLevelSheet(BuildContext context, ActivityLevel currentLevel, ActivityCubit cubit) {
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(shape: BoxShape.circle, boxShadow: [
              BoxShadow(color: const Color(0xFF4361EE).withOpacity(isDark ? 0.08 : 0.05), blurRadius: 60, spreadRadius: 20),
            ]),
          ),
          ...
          Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('${(stats.caloriesProgress * 100).round()}%',
                  style: TextStyle(fontSize: 28.sp, fontWeight: FontWeight.w900,
                      color: const Color(0xFFFF9500), letterSpacing: -1)),
              Text('of goal', style: TextStyle(fontSize: 10.sp, color: context.colors.subText, fontWeight: FontWeight.w500)),
            ],
          ),
        ],
      );
...
class _LogButtonState extends State<_LogButton> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) => GestureDetector(
        ...
        child: ScaleTransition(
          ...
          child: Container(
            height: 56.h,
            decoration: BoxDecoration(
              gradient: const LinearGradient(colors: [Color(0xFF4361EE), Color(0xFF738EFF)],
            ...
            child: Row(
              ...
              children: [
                ...
                SizedBox(width: 10.w),
                Text('Log Activity', style: TextStyle(fontSize: 16.sp, fontWeight: FontWeight.w700, color: Colors.white, letterSpacing: 0.2)),
              ],
            ),
          ),
        ),
