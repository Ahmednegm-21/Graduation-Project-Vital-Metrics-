...
int _stepsGoalFor(OnboardingCubitAllData onboarding) {
  ...
  switch (goal.type) {
    case GoalType.loseWeight: return 12000;
    case GoalType.gainWeight: return 8000;
  }
}
    ...
    return Column(children: [
      ...
      SizedBox(
        ...
        child: PageView(
          ...
          children: [
            _pad(_CaloriesBarChart(
              values:     loaded?.calories.map((v) => v.toDouble()).toList() ??
                          List.filled(7, 0),
            ...
            _pad(_StepsLineChart(
              values:     loaded?.steps.map((v) => v.toDouble()).toList() ??
                          List.filled(7, 0),
              days:       _days,
              isLoading:  isLoading,
              stepsGoal:  stepsGoal,
            ...
            _pad(_BurnedHorizontalChart(
              values:     loaded?.burned.map((v) => v.toDouble()).toList() ??
                          List.filled(7, 0),
              days:       _days,
              isLoading:  isLoading,
              todayIndex: todayIndex,
            )),
            _pad(_WaterAreaChart(
              values:     loaded?.waterMl.map((v) => v / 1000.0).toList() ??
                          List.filled(7, 0),
              days:       _days,
              isLoading:  isLoading,
              todayIndex: todayIndex,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < pts.length; i++) {
      if (i == 0) {
        ...
        final cp = Offset((pts[i - 1].dx + pts[i].dx) / 2,
            (pts[i - 1].dy + pts[i].dy) / 2);
        fill.quadraticBezierTo(
            pts[i - 1].dx, pts[i - 1].dy, cp.dx, cp.dy);
      }
    }
    ...
    for (int i = 0; i < pts.length; i++) {
      final isToday = i == todayIndex;
      canvas.drawCircle(pts[i], isToday ? 5 : 3.5,
          Paint()..color = isToday ? color : color.withOpacity(0.6));
      canvas.drawCircle(pts[i], isToday ? 3 : 2,
          Paint()
            ..color =
        ...
        child: Column(children: [
          ...
          Row(
            ...
            children: List.generate(7, (i) {
              ...
              return Column(children: [
                if (values[i] > 0)
                  Text('${values[i].toStringAsFixed(1)}L',
                      style: TextStyle(
                          fontSize: 7,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    while (x < size.width) {
      canvas.drawLine(Offset(x, goalY),
          Offset(math.min(x + 6, size.width), goalY), dash);
      x += 10;
    }
  }
...
class _ActivitySummaryCard extends StatelessWidget {
  ...
  const _ActivitySummaryCard(
      {required this.fitness, this.stepsGoal = 10000});

  @override
  Widget build(BuildContext context) {
    final isLoading = fitness is FitnessSnapshotLoading ||
        fitness is FitnessSnapshotInitial;
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(label,
                style: TextStyle(
                    fontSize: 9, color: context.colors.subText)),
          ]),
        ]),
      );
}
        ...
        child: Stack(clipBehavior: Clip.none, children: [
          Container(
              ...
              child: Icon(CupertinoIcons.bell_fill,
                  color: isDark ? _orange : _blue, size: 20)),
          if (_unread > 0)
            Positioned(
              top: -4,
              right: -4,
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(label,
                style: TextStyle(
                    color: context.colors.subText, fontSize: 11)),
            Text(value,
                style: TextStyle(
                    color: context.colors.text,
                    fontWeight: FontWeight.bold,
      ...
      child: Column(children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
          ...
          _MacroChip(
              ...
              color: _green),
          _MacroChip(
              label: 'Fat', value: '${cal.totalFat}g', color: _red),
        ]),
      ]),
    );
  }
...
class _MacroChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(children: [
        ...
        Text(label,
            style:
                TextStyle(color: context.colors.subText, fontSize: 11)),
      ]);
}
