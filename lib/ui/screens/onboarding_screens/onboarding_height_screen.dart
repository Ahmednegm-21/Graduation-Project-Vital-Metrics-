...
const double _kAvatarBaseSize = 220.0; // circle base diameter (was 168)
...
class OnboardingHeight extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: const _OnboardingHeightView(),
    );
  }
}

class _OnboardingHeightView extends StatefulWidget {
  const _OnboardingHeightView();

  @override
  State<_OnboardingHeightView> createState() => _OnboardingHeightViewState();
}

class _OnboardingHeightViewState extends State<_OnboardingHeightView>
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<HeightCubit, HeightState>(
      builder: (context, state) {
        if (!_heightController.value.composing.isValid) {
          _heightController.text = state.height.toInt().toString();
        }

        final min = OnboardingConfig.heightConfig['min']!;
        final max = OnboardingConfig.heightConfig['max']!;
        final t = ((state.height - min) / (max - min)).clamp(0.0, 1.0);
        ...
        return Scaffold(
          ...
          body: Column(
            children: [
              _StepHeader(
                accent: accent,
                progressValue: OnboardingConfig.getProgressValue('height'),
                stepLabel: '$_kThisStep / $_kTotalSteps',
                eyebrow: 'YOUR PROFILE',
                title: 'What is your\nheight?',
                subtitle: 'This helps us calculate your ideal metrics',
              ),
                ...
                child: FadeTransition(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          // ── Avatar with idle float + height-aware scale ──
                          AnimatedBuilder(
                          ...
                          Text('What is your height?', style: AppTextStyles.h3),
                          SizedBox(height: 6.h),
                          Text(
                          ...
                          Row(
                            ...
                            children: [
                              _StepperButton(
                                ...
                                accent: accent,
                                onTap: () {
                                  final newHeight = (state.height - 1).clamp(
                                    OnboardingConfig.heightConfig['min']!,
                                    OnboardingConfig.heightConfig['max']!,
                                  );
                                  context.read<HeightCubit>().updateHeight(
                                    newHeight,
                                  );
                                },
                              ),
                              ...
                              SizedBox(
                                ...
                                child: AnimatedSwitcher(
                                  ...
                                  child: Row(
                                    key: ValueKey(state.height.toInt()),
                                    mainAxisAlignment: MainAxisAlignment.center,
                                    crossAxisAlignment:
                            ...
                            child: TextField(
                              ...
                              onSubmitted: (value) {
                                ...
                                if (v != null && v >= min && v <= max) {
                                  context.read<HeightCubit>().updateHeight(v);
                                } else {
              ...
              Padding(
                ...
                child: _ContinueButton(
                  ...
                  enabled: true,
                  onPressed: () {
                    context.read<OnboardingCubitAllData>().setHeight(
                      state.height,
                    );
                    context.push('/weight');
                  },
...
// ─── Continue button (shared visual language with gender screen) ───────────
