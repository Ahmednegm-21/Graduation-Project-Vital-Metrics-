...
/// Shows a detailed side-by-side comparison between the original food
/// and a swap alternative.
...
class _CompareSheetState extends State<_CompareSheet>
  ...
  // Each macro bar animates from 0 → actual value
  ...
  void initState() {
    ...
    _barAnim = CurvedAnimation(parent: _ctrl, curve: const Interval(0.3, 1.0, curve: Curves.easeOutCubic));
  }
...
class _MacroCompareRow extends StatelessWidget {
  ...
  final Color color;
  final List<Color> altGradient;
  final double progress; // 0→1 for bar animation
  ...
  Widget build(BuildContext context) {
    ...
    // Who wins? lower is better for calories/carbs/fats, higher for protein/fiber
    final higherIsBetter = label == 'Protein' || label == 'Fiber';
    final altWins = higherIsBetter
        ? altValue > origValue
        : altValue < origValue;
    final diff    = (altValue - origValue).abs();
    final diffStr = diff < 0.5 ? '≈ same' : (altWins ? '▼' : '▲') +
        ' ${diff.round()}$unit';
