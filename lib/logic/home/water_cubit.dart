...
class WaterCubit extends Cubit<WaterState> {
  ...
  Future<void> setGoalFromProfile({
    ...
    if (calculatedMl != state.goalMl) {
      ...
      print('[WaterCubit] goal updated from profile => $calculatedMl ml'
  ...
  Future<void> _loadLocalCache() async {
    try {
      ...
      final consumedMl = prefs.getInt(_kConsumedMl) ?? 0;
      final cachedIntakes = prefs.getStringList(_kTodayIntakes) ?? [];
      ...
      print('[WaterCubit] LOCAL CACHE => $consumedMl ml, goal=$goalMl ml');
    } catch (e) {
  ...
  Future<void> _saveLocalCache() async {
    try {
      ...
      await prefs.setInt(_kDrinkMl, state.drinkAmountMl);
      await prefs.setString(_kUnit, state.unit);
      await prefs.setInt(_kConsumedMl, state.consumedMl);

      final today = DateTime.now().toIso8601String().split('T').first;
      await prefs.setString(_kLastSavedDate, today);

      final encoded = state.todayIntakes.map((e) {
        return jsonEncode({
          'id': e.id,
          'amountMl': e.amountMl,
      ...
      await prefs.setStringList(_kTodayIntakes, encoded);

      print('[WaterCubit] CACHE SAVED => ${state.consumedMl} ml, goal=${state.goalMl} ml');
    } catch (e) {
  ...
  Future<void> _fetchTodayIntakes() async {
    try {
      ...
      final backendConsumed =
          todayIntakes.fold<int>(0, (sum, e) => sum + e.amountMl);

      final finalConsumed = backendConsumed > state.consumedMl
          ? backendConsumed
          : state.consumedMl;
      ...
      print('[WaterCubit] BACKEND => $backendConsumed ml');
      print('[WaterCubit] FINAL => $finalConsumed ml');
    } catch (e) {
  ...
  Future<void> drink() async {
    ...
    final previousConsumed = state.consumedMl;
    final updatedConsumed = state.consumedMl + state.drinkAmountMl;

    emit(state.copyWith(consumedMl: updatedConsumed));
    await _saveLocalCache();
    ...
    try {
      ...
      emit(state.copyWith(consumedMl: previousConsumed));
      await _saveLocalCache();
    } finally {
      _isDrinking = false;
    }
  ...
  Future<void> removeDrink() async {
    ...
    final updated =
        (state.consumedMl - state.drinkAmountMl).clamp(0, 999999);
    final updatedList = List<WaterModel>.from(state.todayIntakes);
    if (updatedList.isNotEmpty) updatedList.removeLast();
    emit(state.copyWith(consumedMl: updated, todayIntakes: updatedList));
    await _saveLocalCache();
  }
